...
export const FeaturesComponent = () => {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(inner, 10);
  ...
  return (
    <div className="features" style={{ borderRadius: 60 }}>
      <div className="position-relative w-100 mt-5 pt-3">
        <EachImage activeIndex={activeItem} />
      </div>
      <div style={{ textAlign: "center", marginTop: 40 }}>
        <h2 style={{ fontWeight: 500, fontSize: 32, marginBottom: 16 }}>
          {GEMSContent.title}
        </h2>
        <p
          style={{
      ...
      {/* <Carousel controls={false} indicators={false} className="w-100">
        {zoqqFeatures?.map((val, index) => (
          <Carousel.Item interval={500} key={"carousel_item_${index}"}>
            <div className="illustration-parent d-flex flex-column justify-content-center align-item-center">
              <p className="transact-across-190countries">{val?.title}</p>
              <div className="benefit-from-local">{val?.subtitle}</div>
            </div>
          </Carousel.Item>
        ))}
      </Carousel> */}
